...
DEFAULT_CONFIG: dict[str, Any] = {
    ...
    "sources": {"yts": {"enabled": True, "priority": 1}, "nyaa": {"enabled": True, "priority": 2}, "1337x": {"enabled": True, "priority": 3}, "tgx": {"enabled": True, "priority": 4}},
    "custom_sources": [
        {
            "name": "Bitsearch (Torznab via Prowlarr/Jackett)",
            "source_type": "torznab",
            "base_url": "http://prowlarr:9696/1/api?t=search&apikey=SUA_API_KEY",
            "enabled": False,
            "categories": ["movie", "series", "anime"],
            "notes": "Recomendado para conteúdo PT-BR via filtros de release profile: PTBR, DUBLADO, DUAL AUDIO",
        },
        {
            "name": "Torrentio BR (Torznab bridge)",
            "source_type": "torznab",
            "base_url": "http://prowlarr:9696/2/api?t=search&apikey=SUA_API_KEY",
            "enabled": False,
            "categories": ["movie", "series", "anime"],
            "notes": "Use bridge Torznab no Prowlarr/Jackett para indexadores com foco em PT-BR.",
        },
        {
            "name": "Indexer PT-BR custom (Torznab)",
            "source_type": "torznab",
            "base_url": "http://prowlarr:9696/3/api?t=search&apikey=SUA_API_KEY",
            "enabled": False,
            "categories": ["movie", "series", "anime"],
            "notes": "Template para adicionar qualquer indexador dublado PT-BR.",
        },
    ],
...
LANGUAGE_PATTERNS = {
    "dubbed_pt_br": ["dublado", "dual audio", "dual-audio", "dual_audio", "pt-br", "ptbr", "portugues", "nacional", "multi", "lat"],
    "subbed": ["legendado", "subbed", "softsub", "hardsub", "legenda", "subtitle"],
    "original_en": ["english", "eng", "original", "en-us", "en"],
}
...
def matches_quality(text: str, wanted: str) -> bool:
    if not wanted or wanted == "any":
        return True
    return detect_quality(text) == wanted
...
def matches_language(text: str, wanted: str) -> bool:
    ...
    if wanted == "original_en":
        return any(p in t for p in LANGUAGE_PATTERNS["original_en"])
    patterns = LANGUAGE_PATTERNS.get(wanted, [])
    return any(normalize_query(p) in t for p in patterns)
...
async def search_nyaa(query: str, limit: int) -> list[TorrentResult]:
    ...
    for i,e in enumerate(feed.entries[:limit]):
        link = e.get("link", "")
        info_hash = e.get("nyaa_infohash", "")
        magnet = link if str(link).startswith("magnet:") else build_magnet(info_hash, e.get("title", ""))
        out.append(TorrentResult(id=f"nyaa-{e.get('id') or i}", title=e.get("title",""), quality=None,size=e.get("nyaa_size",""),seeders=int(e.get("nyaa_seeders",0) or 0),leechers=int(e.get("nyaa_leechers",0) or 0),source="Nyaa",magnet=magnet,torrent_url=link,type="anime"))
    return out
...
async def search_tgx(query: str, limit: int) -> list[TorrentResult]:
    """TorrentGalaxy RSS (best effort)."""
    url = f"https://torrentgalaxy.to/rss?search={urllib.parse.quote(query)}"
    try:
        async with http_client() as cli:
            r = await cli.get(url)
            r.raise_for_status()
    except Exception as exc:
        raise RuntimeError(f"TorrentGalaxy indisponível: {exc}")
    ...
    for i, e in enumerate(feed.entries[:limit]):
        ...
        out.append(TorrentResult(
            id=f"tgx-{i}-{normalize_query(title)[:32]}",
            title=title,
            source="TGx",
            quality=detect_quality(title),
            seeders=0,
            leechers=0,
            magnet=link if str(link).startswith("magnet:") else None,
            torrent_url=link,
        ))
    return out
...
async def search_torrents(query:str=Query(...), type:str="any", language:str="any", quality:str="any", max_results:int=20):
    ...
    for n, _ in enabled:
        if n == "yts":
            ...
            src_results, warning = await run_source_with_fallback_queries(
                lambda q: search_tgx(q, max_results), expanded, "TGx"
            )
        else:
    ...
    for r in deduped:
        hay = f"{r.title} {r.quality or ''} {r.language_hint or ''}"
        if not matches_quality(hay, quality):
            continue
        if not matches_language(hay, language):
            continue
        if type == "movie" and r.source == "Nyaa":
            continue
        if type == "anime" and r.source == "YTS":
            continue
        filtered.append(r)
    prio={k:v.get("priority",99) for k,v in sources.items()}
    filtered.sort(key=lambda r: score_result(r, expanded, quality, prio.get(r.source.lower(),50)), reverse=True)
    return {"query":query,"expanded_queries":expanded,"count":len(filtered[:max_results]),"results":[r.model_dump() for r in filtered[:max_results]],"warnings":warnings}
